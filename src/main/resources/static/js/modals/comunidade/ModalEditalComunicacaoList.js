const ModalEditalComunicacaoList = {
        props: ['data', 'id'],
        data: function () {
            return {
            	data: this.data,
        ...
        methods:{
	    	      ...
	    	      .then(
	    	    		  response => {
			    	    	  if(response.status == 200){
			    	    		  ...
			    	    		  this.data = response.data.editaisComunicacao
			    	    	  }
			    	      })
		    	     .catch(error => {
        	...
        	confirmCustomDelete(id) {
            	this.$buefy.dialog.confirm({
                    ...
                    onConfirm: () => {
                    	axios
              		  .delete(`/sigequi/api/v1/editaiscomunicacoes/${id}`)
              		  .then(response =>{
        	...
        	saveDocumento(){
	    		...
	    		axios
	    	      .post("/sigequi/editais/"+this.selected.id+"/documento", 
        ...
        mounted() {
            this.titulo="Edital Comunicação"
       	},
	    template: `<div>
	                <div class="modal-card">
	                    <header class="modal-card-head">
	                        <p class="modal-card-title">
	                        <i class="icon-interface-windows" style="font-size: 30px;"></i> Edital Comunicação
	                        </p>
	                    </header>
	                    <section class="modal-card-body"> 
		                    ...
		                    >
			                    <template slot-scope="props">
				                    ...
				                    <b-table-column field="numeroFamilias" label="Número Famílias">
				                        {{ props.row.numeroFamilias }}
				                    </b-table-column>
				                    
				                    <b-table-column field="areaHaEdital" label="Área Edital">
				                        {{ props.row.areaHaEdital }}
				                    </b-table-column>
				                    
				                    <b-table-column field="" label="">
				                    	<a :href="'/sigequi/editais/'+props.row.id+'/documento'" style="color: #fff" 
