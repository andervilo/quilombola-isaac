const ModalAssentamentoEstadual = {
	        props: ['territorio'],
	        data:function(){
	        	return{
	        		object:{
	        ...
	        methods:{
	        	getObjeto(){
	        		axios
		      	      ...
		      	      .then(response => {
		      	    	if(response.data.assentamentoEstadual != null){
			      	  		this.isObjetoSetado = true
	        				this.object = response.data.assentamentoEstadual
                ...
                save(setado) {
		        	...
		        	if(setado){
		        		crudService.update(URL_BASE+"/comunidades/"+this.territorio.id+"/assentamento-estadual", "", this.object, r => {
	            		...
	            		crudService.create(URL_BASE+"/comunidades/"+this.territorio.id+"/assentamento-estadual", this.object, r => {
	            			console.log(r)
	            			if(r.status == 200){
		        				this.$buefy.toast.open({
		                            duration: 3000,
	                ...
	                <div class="modal-card" >
	                    <header class="modal-card-head">
	                        <p class="modal-card-title">
		                    	<i class="icon-interface-windows" style="font-size: 30px;"></i> Assentamento Estadual
